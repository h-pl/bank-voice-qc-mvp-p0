...
function verification(count=2){let s=run(start(),'WO-1039','publish',{remedy:true,goal:'改进服务说明',standard:'完整说明并避免消极表达',sampleCount:count,observation:'整改后同业务样例'}),id=s.remedies.at(-1).id;s=run(as(s,'A1048'),id,'accept_remedy');s=run(s,id,'sample_calls');s=run(s,id,'material',{samples:[s.calls.at(-1).id]});return[s,id];}
...
test('R02: executor options and command reject a different agent, inspectors get scoped access on assignment',()=>{
 const s=start();assert.ok(!supplementExecutors(s,'F-1041').some(p=>p.id==='A1186'));
 assert.throws(()=>run(s,'F-1041','supplement',{owner:'A1186'}),/无此通话/);
 for(const p of supplementExecutors(s,'F-1041')){const next=run(s,'F-1041','supplement',{owner:p.id}),sp=next.supplements.at(-1);assert.ok(canSee(as(next,p.id),sp.id));assert.ok(actions(as(next,p.id),sp.id).includes('reply'));}
});
test('R03: active appeal cannot be overwritten from finding or another review and can still withdraw',()=>{
 const s=start();assert.ok(!actions(s,'F-1033').includes('followup'));assert.throws(()=>run(s,'F-1033','followup'),/不允许/);
...
test('R03: ordinary follow-up conclusion change terminates old personnel remedy and its supplements',()=>{
 let s=start();s=run(s,'F-1034','followup',{owner:'A1186'});s=run(s,'F-1034','dismiss',{value:'insufficient'});
 assert.equal(entity(s,'REC-1034').status,'terminated');assert.match(entity(s,'REC-1034').terminationReason,/依据不充分/);assert.equal(latest(entity(s,'F-1034')).version,2);
});
test('R04: supervisor selected evidence is immutable per conclusion version',()=>{
 let s=start(),r=entity(s,'WO-1039'),fid=r.findingIds[0],original=[...entity(s,fid).evidence];
 s=run(s,r.id,'publish',{opinions:{[fid]:{value:'false_positive',note:'依据第四片段进行主管判断',evidence:[3]}}});
 assert.deepEqual(latest(entity(s,fid)).evidence,[3]);assert.deepEqual(entity(s,fid).evidence,original);
 s=run(s,fid,'followup');s=run(s,fid,'dismiss',{value:'insufficient',evidence:[1]});
 assert.deepEqual(entity(s,fid).conclusions.map(c=>c.evidence),[[3],[1]]);
});
test('R05: failed round retains material, samples and standards while new round starts empty',()=>{
 let[s,id]=verification(1);const materials=structuredClone(entity(s,id).materials);s=run(as(s,'Q01'),id,'verify',{value:'fail'});s=run(as(s,'S01'),id,'return_remedy');
...
test('R07: two confirmed problems may have different dispositions, creating only one remedy',()=>{
 let s=start();const r=entity(s,'WO-1039');const [a,b]=r.findingIds;
 s=run(s,r.id,'publish',{opinions:{[a]:{value:'risk',note:'第一项已有充分事实依据',evidence:[2]},[b]:{value:'risk',note:'第二项需持续改进行为',evidence:[3]}},dispositions:{[a]:{remedy:false,noRemedy:'本通已完成纠正处理'},[b]:{remedy:true,goal:'改善业务说明',standard:'完整说明业务条件',owner:'Q02',dueAt:'2026-09-18T12:00:00+08:00',sampleCount:2,observation:'后续同业务样例'}}});
 assert.equal(s.remedies.filter(r=>r.findingId===a).length,0);assert.equal(s.remedies.filter(r=>r.findingId===b).length,1);assert.equal(s.remedies.at(-1).inspector,'Q02');assert.equal(s.remedies.at(-1).dueAt,'2026-09-18T12:00:00+08:00');assert.equal(s.remedies.at(-1).originalDueAt,'2026-09-18T12:00:00+08:00');assert.ok(latest(entity(s,a)).noRemedy);
});
test('R07: completed existing fact can be linked without a new Finding or changing its source',()=>{
 let s=run(start(),'CALL-1035','spotcheck',{scope:'敏感信息补充核对'});const r=s.reviews.at(-1),count=s.findings.length;
 s=run(as(s,'Q01'),r.id,'link_finding',{findingId:'F-1035'});assert.ok(entity(s,r.id).findingIds.includes('F-1035'));assert.equal(s.findings.length,count);assert.equal(entity(s,'F-1035').source,'auto');
 assert.throws(()=>run(s,r.id,'link_finding',{findingId:'F-1035'}),/已经关联/);
});
...
test('R10/R12: pending reads are tasks, paused remedies are not, next business action is primary',()=>{
 const s=as(start(),'A1048');assert.ok(isTodo(s,entity(s,'F-1035')));assert.ok(!isTodo(s,entity(s,'REC-1033')));assert.equal(primaryAction(s,'AP-1033'),undefined);
 assert.equal(primaryAction(as(s,'Q01'),'WO-1038'),'submit_review');assert.equal(primaryAction(as(s,'S01'),'WO-1039'),'publish');
});
test('C03: compound acceptance is atomic, records both events, and request retry is idempotent',()=>{
 const s=start();const cmd={id:'AP-1033',rev:entity(s,'AP-1033').rev,action:'accept_assign',requestId:'compound',input:{owner:'Q02',dueAt:due,note:'由其他质检员进行独立核查'}};
...
test('R09: appeal review evidence request returns via supervisor to the same inspector',()=>{
 ...
 s=run(as(s,'Q02'),id,'request_evidence',{note:'需要坐席补充自动核验依据'});
 s=run(as(s,'S01'),id,'supplement',{owner:'Q01'});const sp=s.supplements.at(-1);
 assert.equal(sp.target,'AP-1033');assert.equal(sp.executor,'A1048');
 s=run(as(s,'A1048'),sp.id,'reply');s=run(as(s,'S01'),sp.id,'receive_supplement');
