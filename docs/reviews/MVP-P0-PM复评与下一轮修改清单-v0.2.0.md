...
**状态更新（2026-09-13）：N05/N06 已按用户的 UI 优化要求在 v0.2.1 修复并通过浏览器验收；N01—N04 及末尾范围取舍仍待后续处理。** 上一轮 R01—R16 的实施与 38 项通过记录见 [v0.2.0 验收](版本迭代与验收-v0.2.0.md)。新发现是扩展组合路径与界面恢复检查的结果，不能以已有回归通过推导所有组合均无漏洞。
...
N01—N06 均建议采纳，范围限于本报告，继续作为高保真 MVP。建议顺序 **N01 → N02/N03 → N04/N05 → N06**。
...
这些是下一轮建议，尚未更新现行 PRD 决策或修改应用代码；发布标签 v0.2.0-mvp-p0 保持不动。
