...
- 业务资源以分类表格为入口，SearchField、Tabs、Pagination 继续复用。点击资源名称以 resource Modal 变体打开右侧预览；完整详情独立呈现，长内容在 InlineFormSurface 页面编辑。表格筛选、分页与位置保留；详情版本与引用明确分开。
...
正文单一滚动，标题与底部动作固定；校验反馈位于正文之后、按钮之前，必填字段仍聚焦并标记错误。证据默认展示按时间排序的已选原文，调整时展开同一正文内完整列表，收起保留选择；无选中项初始展开。操作弹窗不因遮罩点击关闭，Escape/关闭/取消沿用现有退出行为并恢复触发者焦点。资源页内编辑仍使用既有表单布局与离开保护。
