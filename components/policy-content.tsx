...
export function ResourceContent({ resource, snapshot }: { resource: Resource; snapshot: ResourceVersion }) {
  ...
  if (resource.type === "SOP") return <ol className="resource-steps" aria-label="SOP 步骤">{entries.map((entry, i) => <li key={i}><span aria-hidden="true">{String(i + 1).padStart(2, "0")}</span><p>{entry.replace(/^\d+[.、．]\s*/, "")}</p></li>)}</ol>;
...
const versionFields = { content: "内容", scope: "适用业务", role: "适用角色", exception: "例外说明", threshold: "静默阈值", trigger: "提醒触发", resources: "资源引用" };
...
export function VersionHistory({ current, selected, onSelect }: { current: Rule | Resource; selected: number; onSelect: (version: number) => void }) {
  return <section className="policy-section policy-history" aria-label="版本记录">
    ...
    {current.versions.length ? <div className="version-entries">{[...current.versions].reverse().map(v => <button type="button" key={v.version} aria-pressed={selected === v.version} className={`version-entry ${selected === v.version ? "selected" : ""}`} onClick={() => onSelect(v.version)}><span className="version-entry-id">V{v.version}<Badge tone={v === current.versions.at(-1) ? "success" : "neutral"}>{v === current.versions.at(-1) ? "当前生效" : "历史版本"}</Badge></span><span><b>{versionSummary(v, current.versions.find(p => p.version === v.version - 1))}</b><small>{stamp(v.at)}{selected === v.version ? " · 正在查看" : " · 查看此版本"}</small></span></button>)}</div> : <p className="subtle">尚未发布。草稿通过检查并发布后生成首个版本。</p>}
  </section>;
}
