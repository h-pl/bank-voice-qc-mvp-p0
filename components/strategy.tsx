...
export function Strategy({ state, view, onAction, onSubmit, onOpen, focus, detailOnly = false }: {
  ...
  return <>
    <div className={`policy-layout ${resources ? "resource-layout" : "rule-layout"} ${focus ? "has-selection" : ""} ${detailOnly ? "policy-full-detail" : ""}`}>
      ...
      <section className="policy-detail panel" aria-label={resources ? "资源详情" : "规则详情"}>
        {current && snapshot ? <>
          ...
          {rule ? <>
            {rule.editable && <section ref={parameterRef} className="policy-section parameter-section">
              <div className="policy-section-title"><h3>可调整参数</h3>{canEdit ? <Button primary icon="edit" onClick={() => setEditing(current.id)}>{current.draft ? "修改草稿" : "编辑参数"}</Button> : <span>{historical ? "历史参数" : manager ? rule.editable ? "编辑中" : "固定规则" : "当前身份只读"}</span>}</div>
    ...
    {editing === current?.id && rule && canEdit && <RuleEditor key={rule.id} rule={rule} onSubmit={onSubmit} onDone={()=>{restoreParameterFocus.current=true;setEditing(undefined);}}/>}
    {catalog && (
        <Modal
          title="完整 16 项指标与页面分工"
          onClose={() => setCatalog(false)}
          footer={<Button primary onClick={()=>setCatalog(false)}>了解</Button>}
        >
          <p className="callout">
