...
基线 `v0.0.0-draft`；当前交付 `v0.2.3-mvp-p0`；分支 `codex/mvp-p0-ui-refinement`（保留此前所有标签）。原项目目录保持独立。

当前节点：质量报表视觉与分析交互迭代，详见 [本轮验收](docs/reviews/报表UI迭代与验收-v0.2.3.md)。本轮完成分析模块的质检概览、问题分布、坐席/班组、申诉整改报表；其余六模块随快照保留。
